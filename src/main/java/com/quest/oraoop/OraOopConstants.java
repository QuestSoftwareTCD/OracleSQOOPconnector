...
public class OraOopConstants {

    public static final String ORAOOP_PRODUCT_NAME = "Quest® Data Connector for Oracle and Hadoop";
    ...
    // The name of the Oracle RAC service each mapper should connect to, via their dynamically generated JDBC URL...
    ...
    // The properties are used internally by OraOop to indicate the schema and name of
    ...
    // The minimum number of mappers required for OraOop to accept the import job...
    ...
    // The minimum number of mappers required for OraOop to accept the export job...
  ...
  public static final String ORAOOP_ORACLE_DATA_CHUNKS_QUERY = "oraoop.oracle.data.chunks.query";
    ...
    // The minimum number of active instances in an Oracle RAC required for OraOop to use dynamically generated JDBC URLs...
    ...
    // The name of the data_chunk_id column the OraOop appends to each (import) query...
    ...
    // Pseudo-columns added to an partitioned export table (created by OraOop from a template table)
    // to store the partition value and subpartition value. The partition value is the sysdate when
    ...
    // The string we want to pass to dbms_application_info.set_module() via the "module_name" parameter...
    ...
    // Identifies an existing Oracle table used to create a new table as the destination of a Sqoop export.
    // Hence, use of this property implies that the "-table" does not exist in Oracle and OraOop should create it.
    ...
    // If ORAOOP_EXPORT_CREATE_TABLE_TEMPLATE has been specified, then this flag indicates whether the created Oracle
    ...
    // Indicates (internally) the the export table we're dealling with has been paritioned by OraOop...
    public static final String EXPORT_TABLE_HAS_ORAOOP_PARTITIONS = "oraoop.export.table.has.oraoop.partitions";

    // When using the Oracle hint... /* +APPEND_VALUES */ ...a commit must be performed after each batch insert.
    // Therefore, the batches need to be quite large to avoid a performance penality (for the 'extra' commits).
    ...
    // When OraOop creates a table for a Sqoop export (from a template table) and the table contains partitions,
    // this is the prefix of those partition names. (This also allows us to later identify partitions that OraOop
    // created.)
    ...
    // The format string used to turn a DATE into a string for use within the names of Oracle objects
    // that we create. For example, temporary tables, table partitions, table subpartitions...
    public static final String ORACLE_OBJECT_NAME_DATE_TO_STRING_FORMAT_STRING = "yyyymmdd_hh24miss";

    // Indicates whether to perform a "merge" operation when performing a Sqoop export.
    ...
    // Flag used to indicate that the Oracle table contains at least one column of type BINARY_DOUBLE...
    public static final String TABLE_CONTAINS_BINARY_DOUBLE_COLUMN = "oraoop.table.contains.binary.double.column";
    // Flag used to indicate that the Oracle table contains at least one column of type BINARY_FLOAT...
    public static final String TABLE_CONTAINS_BINARY_FLOAT_COLUMN = "oraoop.table.contains.binary.float.column";

    // The storage clause to append to the end of any CREATE TABLE statements we execute for temporary Oracle tables...
    public static final String ORAOOP_TEMPORARY_TABLE_STORAGE_CLAUSE = "oraoop.temporary.table.storage.clause";

    // The storage clause to append to the end of any CREATE TABLE statements we execute for permanent (export) Oracle tables...
    ...
    public static final String SUPPORTED_IMPORT_ORACLE_DATA_TYPES_CLAUSE = "(DATA_TYPE IN ("+
            ...
            // "'UNDEFINED',"+
            "'URITYPE',"+
            // "'UROWID',"+ //<- SqlType = 1111 = "OTHER" Not supported as "AAAAACAADAAAAAEAAF" is being returned as "AAAAAAgADAAAA"
            "'VARCHAR2'"+ // <- Columns declared as VARCHAR are listed as VARCHAR2 in dba_tabl_columns
            // "'XMLTYPE',"+
            ")" + " OR DATA_TYPE LIKE 'INTERVAL YEAR(%) TO MONTH'" + " OR DATA_TYPE LIKE 'INTERVAL DAY(%) TO SECOND(%)'" + " OR DATA_TYPE LIKE 'TIMESTAMP(%)'" + " OR DATA_TYPE LIKE 'TIMESTAMP(%) WITH TIME ZONE'"
            + " OR DATA_TYPE LIKE 'TIMESTAMP(%) WITH LOCAL TIME ZONE'" + ")";

    public static final String SUPPORTED_EXPORT_ORACLE_DATA_TYPES_CLAUSE = "(DATA_TYPE IN ("+
            // "'BFILE',"+
            "'BINARY_DOUBLE',"+ 
            "'BINARY_FLOAT',"+
            ...
            "'DATE',"+ 
            "'FLOAT',"+ 
            // "'LONG',"+    //<- "create table as select..." and "insert into table as select..." do not work when a long column exists.
            // "'LONG RAW',"+
            // "'MLSLABEL',"+
            "'NCHAR',"+
    ...
    public static class Oracle {
        ...
        public static final String ORACLE_SQL_STATEMENT_COMMENT_TOKEN = "--"; // <- This is how you comment-out a line of SQL text in Oracle.

        public static final String OBJECT_TYPE_TABLE = "TABLE";

        public static final String URITYPE = "URITYPE";
        
        public static final int MAX_IDENTIFIER_LENGTH = 30; //<- Max length of an Oracle name (table-name, partition-name etc.)
        
        public static final String HINT_SYNTAX = "/*+ %s */ "; //Syntax for a hint in Oracle
    }
