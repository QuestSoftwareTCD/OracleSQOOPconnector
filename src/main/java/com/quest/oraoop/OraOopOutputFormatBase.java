...
abstract class OraOopOutputFormatBase<K extends SqoopRecord, V>
    ...
    protected boolean canUseOracleAppendValuesHint(TaskAttemptContext context) {
        ...
        OracleVersion oracleVersion = new OracleVersion(conf.getInt(OraOopConstants.ORAOOP_ORACLE_DATABASE_VERSION_MAJOR, 0)
                                                       ,conf.getInt(OraOopConstants.ORAOOP_ORACLE_DATABASE_VERSION_MINOR, 0)
                                                       ,0
                                                       ,0
                                                       ,"");
        ...
        // If there is a BINARY_DOUBLE or BINARY_FLOAT column, then we'll avoid using
        // the APPEND_VALUES hint. If there is a NULL in the HDFS file, then we'll encounter
        ...
        if(result) {
            boolean binaryDoubleColumnExists = conf.getBoolean(OraOopConstants.TABLE_CONTAINS_BINARY_DOUBLE_COLUMN, false);
            boolean binaryFloatColumnExists = conf.getBoolean(OraOopConstants.TABLE_CONTAINS_BINARY_FLOAT_COLUMN, false);
            if(binaryDoubleColumnExists || binaryFloatColumnExists) {
                result = false;
                LOG.info("The APPEND_VALUES Oracle hint will not be used for the INSERT SQL statement, "+
                         "as the Oracle table contains either a BINARY_DOUBLE or BINARY_FLOAT column.");
            }
        }
    ...
    protected boolean allowUserToOverrideUseOfTheOracleAppendValuesHint(TaskAttemptContext context, boolean useAppendValuesOracleHint) {
        ...
        switch(OraOopUtilities.getOracleAppendValuesHintUsage(conf)) {
            ...
            case OFF:
                result = false;
                LOG.debug(String.format("Use of the APPEND_VALUES Oracle hint has been forced OFF. (It was %s to used)."
                                       ,useAppendValuesOracleHint ? "going" : "not going" ));
                break;
            
            case ON:
                result = true;
                LOG.debug(String.format("Use of the APPEND_VALUES Oracle hint has been forced ON. (It was %s to used)."
                                       ,useAppendValuesOracleHint ? "going" : "not going" ));
                break;
    ...
    protected void updateBatchSizeInConfigurationToAllowOracleAppendValuesHint(TaskAttemptContext context) {
        ...
        int originalBatchesPerCommit = conf.getInt(AsyncSqlOutputFormat.STATEMENTS_PER_TRANSACTION_KEY, 0);
        if (originalBatchesPerCommit != 1) {
            conf.setInt(AsyncSqlOutputFormat.STATEMENTS_PER_TRANSACTION_KEY, 1);
            LOG.info(String.format("The number of batch-inserts to perform per commit has been changed from "+ 
                                   "%d to %d. This is in response to the Oracle APPEND_VALUES hint being used."
                                  ,originalBatchesPerCommit
                                  ,1));
        }

        int originalBatchSize = conf.getInt(AsyncSqlOutputFormat.RECORDS_PER_STATEMENT_KEY, 0);
        int minAppendValuesBatchSize = OraOopUtilities.getMinAppendValuesBatchSize(conf);
        if (originalBatchSize < minAppendValuesBatchSize) {
            conf.setInt(AsyncSqlOutputFormat.RECORDS_PER_STATEMENT_KEY, minAppendValuesBatchSize);
            LOG.info(String.format("The number of rows per batch-insert has been changed from %d to %d. "+ 
                                   "This is in response to the Oracle APPEND_VALUES hint being used."
                                  ,originalBatchSize
    ...
    abstract class OraOopDBRecordWriterBase<K extends com.cloudera.sqoop.lib
        ...
        protected boolean tableHasMapperRowNumberColumn;          //<- Whether the export table contain the column ORAOOP_MAPPER_ROW 
        protected long mapperRowNumber;                           //<- The 1-based row number being processed by this mapper. It's inserted into the "ORAOOP_MAPPER_ROW" column
        
        public OraOopDBRecordWriterBase(TaskAttemptContext context
                                       ,int mapperId)
            ...
            LOG.info(String.format("This record writer is connected to Oracle via the JDBC URL: \n" + "\t\"%s\"\n"+ 
                                  ...
                                  ,((OracleConnection) connection).getURL()
        ...
        protected void setOracleTableColumns(OracleTableColumns oracleTableColumns) {
            
            this.oracleTableColumns = oracleTableColumns;
            this.tableHasMapperRowNumberColumn = this.oracleTableColumns
                                                     .findColumnByName(OraOopConstants.COLUMN_NAME_EXPORT_MAPPER_ROW) != null;
        }
        ...
        @Override
        protected PreparedStatement getPreparedStatement(List<SqoopRecord> userRecords) 
            throws SQLException {

            oracle.jdbc.driver.OracleConnection connection = (oracle.jdbc.driver.OracleConnection) this.getConnection();

            String sql = getBatchSqlStatement();
            LOG.debug(String.format("Prepared Statement SQL:\n%s", sql));

            oracle.jdbc.OraclePreparedStatement statement;

            try {
                // Synchronize on connection to ensure this does not conflict
                // with the operations in the update thread.
                synchronized (connection) {
                    statement = (oracle.jdbc.OraclePreparedStatement) connection.prepareStatement(sql);
                }
        ...
        protected String getInsertStatement(int numRows) {
            ...
            throw new UnsupportedOperationException(String.format("%s should not be called, as %s operates in batch mode."
                                                                 ,OraOopUtilities.getCurrentMethodName()
        ...
        protected String getBatchInsertSqlStatement(String oracleHint) {
            ...
            /*
             * NOTE: "this.oracleTableColumns" may contain a different list of columns than "this.getColumnNames()". 
             * This is because: 
             * (1) "this.getColumnNames()" includes columns with data-types that are not supported by OraOop. 
             * (2) "this.oracleTableColumns" includes any pseudo-columns that we've added to the export table 
             * (and don't exist in the HDFS file being read). 
             * For example, if exporting to a partitioned table (that OraOop created), there are two
             * pseudo-columns we added to the table to identify the export job and the mapper.
             */
        ...
        protected void configurePreparedStatementColumns(OraclePreparedStatement statement, Map<String, Object> fieldMap) 
            ...
            if(this.tableHasMapperRowNumberColumn) {
                bindValueName = columnNameToBindVariable(OraOopConstants.COLUMN_NAME_EXPORT_MAPPER_ROW).replaceFirst(":", "");
                statement.setLongAtName(bindValueName, this.mapperRowNumber);
            ...
            while (columnNameIterator.hasNext()) {
                ...
                switch (oracleType) {
                    ...
                    case oracle.jdbc.OracleTypes.NUMBER:
                        statement.setBigDecimalAtName(bindValueName, (BigDecimal) fieldMap.get(colName));
                        break;

                    case oracle.jdbc.OracleTypes.VARCHAR:
                    ...
                    case oracle.jdbc.OracleTypes.BINARY_DOUBLE: {
                        Double value = (Double) fieldMap.get(colName);
                        if (value != null)
                            statement.setBinaryDoubleAtName(bindValueName, value);
                        else
                            statement.setObjectAtName(bindValueName, null);
                        break;
                    }

                    case oracle.jdbc.OracleTypes.BINARY_FLOAT: {
                        Float value = (Float) fieldMap.get(colName);
                        if (value != null)
                            statement.setBinaryFloatAtName(bindValueName, value);
                        else
                            statement.setObjectAtName(bindValueName, null);
                        break;
                    }
                    ...
                    case oracle.jdbc.OracleTypes.STRUCT: // <- E.g. URITYPE
                    {
                        if (oracleTableColumn.dataType.equals(OraOopConstants.Oracle.URITYPE)) {
                            ...
                            String msg = String.format("%s needs to be updated to cope with the data-type: %s "+ 
                                                       "where the Oracle data_type is \"%s\"."
                                                      ,OraOopUtilities.getCurrentMethodName()
                                                      ,oracleTableColumn.dataType
                                                      ,oracleType);
                            LOG.error(msg);
                    ...
                    default: {
                        ...
                        Object value = fieldMap.get(colName);
                        statement.setObjectAtName(bindValueName, value);
                    }
                }
        ...
        private void showSqlBatchErrorDetails(Exception exception) {
            ...
            /*
             * Unfortunately, BatchUpdateException.getUpdateCounts() only returns information about UPDATE statements (not INSERT) statements. Since we're only performing INSERT statements, there's no extra information we can provide to the user at
             * this point.
             */
        ...
        protected oracle.sql.DATE getJobSysDate(TaskAttemptContext context) {
            
            Configuration conf = context.getConfiguration();
            return OraOopUtilities.recallOracleDateTime(conf, OraOopConstants.ORAOOP_JOB_SYSDATE);
        }
        ...
        protected OracleTable createUniqueMapperTable(TaskAttemptContext context) 
            ...
            // If this mapper is being reattempted in response to a failure, we need to delete the
        ...
        protected String generateInsertValueForPseudoColumn(String columnName) {
            ...
            if (columnName.equalsIgnoreCase(OraOopConstants.COLUMN_NAME_EXPORT_PARTITION)) {
                ...
                String partitionValueStr = this.getConf().get(OraOopConstants.ORAOOP_EXPORT_PARTITION_DATE_VALUE, null);
            ...
            if (columnName.equalsIgnoreCase(OraOopConstants.COLUMN_NAME_EXPORT_SUBPARTITION)) {
        ...
        protected void logBatchSettings() {
            ...
            int stmtsPerTx = this.getConf().getInt(AsyncSqlOutputFormat.STATEMENTS_PER_TRANSACTION_KEY
