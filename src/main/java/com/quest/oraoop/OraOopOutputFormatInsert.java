...
public class OraOopOutputFormatInsert<K extends SqoopRecord, V>
    ...
    public RecordWriter<K, V> getRecordWriter(TaskAttemptContext context) throws IOException {
        ...
        if(insertMode == InsertMode.ExchangePartition) {
            // NB: "Direct inserts" cannot utilize APPEND_VALUES, otherwise Oracle will serialize
    ...
    public class OraOopDBRecordWriterInsert extends OraOopDBRecordWriterBase {
        ...
        private String subPartitionName;                        //<- The name of the subpartition in the "main table" that this mappers unique table will be exchanged with

        public OraOopDBRecordWriterInsert(TaskAttemptContext context
                                         ,int mapperId
        ...
        protected void getExportTableAndColumns(TaskAttemptContext context) throws SQLException {
            ...
            switch(this.insertMode) {
                ...
                case ExchangePartition: 
                    // This mapper inserts data into a unique table before exchanging it into 
                    ...
                    this.subPartitionName = OraOopUtilities.generateExportTableSubPartitionName(this.mapperId
                                                                                               ,this.getJobSysDate(context)
                                                                                               ,conf);
                    
                    break;                    
                
                default:
                    throw new RuntimeException(String.format("Update %s to cater for the insertMode \"%s\"."
        ...
        private void exchangePartitionUniqueMapperTableDataIntoMainExportTable(TaskAttemptContext context) throws SQLException {
            ...
            try {
                ...
                LOG.info(String.format("Time spent performing an \"exchange subpartition with table\": %f sec."
                                      ,timeInSec));
            }
            catch (SQLException ex) {
                throw new SQLException(String.format("Unable to perform an \"exchange subpartition\" operation for the "+ 
                                                     "table %s, for the subpartition named \"%s\" with the table named \"%s\"."
                                                    ,mainTable.toString()
                                                    ,this.subPartitionName
