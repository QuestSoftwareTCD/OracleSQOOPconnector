...
import com.quest.oraoop.OraOopOracleQueries.CreateExportChangesTableOptions;
...
public class OraOopOutputFormatUpdate<K extends SqoopRecord, V> 
    ...
    public RecordWriter<K, V> getRecordWriter(TaskAttemptContext context) throws IOException {
        ...
        useAppendValuesOracleHint = allowUserToOverrideUseOfTheOracleAppendValuesHint(context, useAppendValuesOracleHint);
    ...
    public class OraOopDBRecordWriterUpdate<K extends SqoopRecord, V> extends OraOopDBRecordWriterBase<K,V> {
        ...
        private String[] updateColumnNames;         //<- The name of the column(s) used to match a row in the HDFS file to a row in the Oracle table. i.e. What as specified in the "--update-key" sqoop argument.
        private UpdateMode updateMode;              //<- The modus operandi of this class. i.e. Whether we update the Oracle table directly, or insert data into a separate table and then apply a SQL MERGE statement. 
        private boolean useAppendValuesOracleHint;  //<- Whether to use the " /*+APPEND_VALUES*/ " hint within the Oracle SQL statement we generate
        private boolean tableHasOraOopPartitions;   //<- Indicates whether the export table has partitions that were creted by OraOop
        private long numberOfRowsSkipped;           //<- The number of rows encountered during configurePreparedStatement() that had a NULL value for (one of) the update columns. This row was therefore skipped.

        public OraOopDBRecordWriterUpdate(TaskAttemptContext context
                                         ,int mapperId
            ...
            this.tableHasOraOopPartitions = conf.getBoolean(OraOopConstants.EXPORT_TABLE_HAS_ORAOOP_PARTITIONS, false);
        }
            ...
            try {
                ...
                if(this.numberOfRowsSkipped > 0)
                    LOG.warn(String.format("%d records were skipped due to a NULL value within one of the update-key column(s).\n"+
                                           "Having a NULL value prevents a record from being able to be matched to a row in the Oracle table."
                                          ,this.numberOfRowsSkipped));
                
                // Now update the "main" export table with data that was inserted into this mapper's table...
                updateMainExportTableFromUniqueMapperTable(context, this.updateColumnNames);
        ...
        private ExportTableUpdateTechnique getExportTableUpdateTechnique() {
            ...
            if(this.tableHasOraOopPartitions) {
                switch(this.updateMode) {
                    ...
                    default:
                        throw new RuntimeException(String.format("Update %s to cater for the updateMode \"%s\"."
        ...
        private void updateMainExportTableFromUniqueMapperTable(TaskAttemptContext context, String[] mergeColumnNames) throws SQLException {
            ...
            switch(exportTableUpdateTechnique) {
                ...
                case UpdateSql:
                    changesTableOptions = CreateExportChangesTableOptions.OnlyRowsThatDiffer;
                    break;

                case ReInsertUpdatedRowsAndNewRows:
                case MergeSql:
                    changesTableOptions = CreateExportChangesTableOptions.RowsThatDifferPlusNewRows; 
                    break;
                    
                default:
                    throw new RuntimeException(String.format("Update %s to cater for the ExportTableUpdateTechnique \"%s\"."
                                                            ,OraOopUtilities.getCurrentMethodName()
            ...
            try {
                int changeTableRowCount = OraOopOracleQueries.createExportChangesTable(this.getConnection()
                                                                                      ,changesTable
                ...
                if(changeTableRowCount == 0) {
                    LOG.debug(String.format("The changes-table does not contain any rows. %s is now exiting."
                                           ,OraOopUtilities.getCurrentMethodName()));
                    return;
                }
                ...
                switch(exportTableUpdateTechnique) {
                    ...
                    case UpdateSql: {
                        ...
                        OraOopOracleQueries.updateTable(this.getConnection()
                                                       ,targetTable
                                                       ,changesTable
                                                       ,this.updateColumnNames
                                                       ,this.getOracleTableColumns()
                                                       ,sysDateTime
                                                       ,this.mapperId
                                                       ,parallelizationEnabled);
                    ...
                    case MergeSql:{
                        ...
                        OraOopOracleQueries.mergeTable(this.getConnection()
                                                      ,targetTable
                                                      ,changesTable
                                                      ,this.updateColumnNames
                                                      ,this.getOracleTableColumns()
        ...
        protected String getBatchSqlStatement() {
            ...
            if (sqlStatement == null) 
                this.sqlStatement = getBatchInsertSqlStatement(this.useAppendValuesOracleHint 
                                                               ? "/*+APPEND_VALUES*/" 
                                                               : "");                     
        ...
        @Override
        void configurePreparedStatement(OraclePreparedStatement statement, List<SqoopRecord> userRecords) 
            throws SQLException {
