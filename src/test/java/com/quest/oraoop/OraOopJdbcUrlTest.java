...
public class OraOopJdbcUrlTest extends OraOopTestCase {
    ...
    public void testParseJdbcOracleThinConnectionString() {
        ...
        catch(JdbcOracleThinConnectionParsingError ex) { Assert.fail("An IllegalArgumentException should be been thrown."); }
        ...
        try {
            ...
            assertTrue("An exception should be thown that tells us there's an incorrect number of fragments in the JDBC URL."
                    , ex.getMessage().toLowerCase().contains("there should be 5 or 6 colon-separated pieces of data in the jdbc url"));
        }
        ...
        try {
            ...
            // This is what we want to happen.
            assertTrue("An exception should be thown that refers to the fact that the thin JDBC driver is not being used."
                    , ex.getMessage().toLowerCase().contains("oracle \"thin\" jdbc driver is not being used"));
            
            assertTrue("An exception should be thown that tells us which JDBC driver was specified."
        ...
        try {
            ...
            assertTrue("The invalid port number should be included in the exception message.", ex.getMessage().toLowerCase().contains("port1521")); 
        }
        ...
        try {
            ...
            assertTrue("The invalid port number should be included in the exception message.", ex.getMessage().toLowerCase().contains("-1521")); 
        }       
        
        // Valid JDBC URL...
        try {
            actual = new OraOopJdbcUrl("JDBC:Oracle:tHiN:@hostname.domain.com.au:1521:dbsid").parseJdbcOracleThinConnectionString();
            Assert.assertEquals("hostname.domain.com.au", actual.host);
            Assert.assertEquals(1521, actual.port);
            Assert.assertEquals("dbsid", actual.sid);
        }
        catch(JdbcOracleThinConnectionParsingError ex) { 
        ...
        try {
            ...
            Assert.assertEquals("hostname.domain.com.au", actual.host);
            Assert.assertEquals(1529, actual.port);
            Assert.assertEquals("dbsid", actual.sid);
        }
        catch(JdbcOracleThinConnectionParsingError ex) { 
            Assert.fail(ex.getMessage()); 
        ...
        try {
            ...
            Assert.assertEquals("hostname", actual.host);
            Assert.assertEquals(1521, actual.port);
            Assert.assertEquals("dbsid", actual.sid);
            Assert.assertEquals(null, actual.service);
        }
        catch(JdbcOracleThinConnectionParsingError ex) { 
            Assert.fail(ex.getMessage()); 
        }   
        ...
        try {
            ...
            Assert.assertEquals("hostname", actual.host);
            Assert.assertEquals(1521, actual.port);
            Assert.assertEquals(null, actual.sid);
            Assert.assertEquals("dbservice.dbdomain", actual.service);
        }
        catch(JdbcOracleThinConnectionParsingError ex) { 
            Assert.fail(ex.getMessage()); 
        }
